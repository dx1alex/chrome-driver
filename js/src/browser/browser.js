...
class Browser extends base_1.Base {
    constructor(wd_options, options) {
        ...
        return this._thisProxy = new Proxy(this, {
            get: (browser, command, r) => {
                ...
                return (...args) => {
                    let lastCommand;
                    if (!new.target._noCommandHistory.includes(command)) {
                        ...
                        console.log(`${command} ${strArgs} ${getDateTime()}\n` + stack);
                    }
    ...
    $(selector) {
        return new Proxy(this, {
            get: (browser, command, r) => {
                if (typeof browser[command] !== 'function' || !Browser._$List.includes(command))
                    return void 0;
                return async (...args) => {
...
Browser._noProxyList = ['$', 'getCommandHistory', 'getCommandHistoryErrors', 'lastError'];
Browser._noCommandHistory = ['waitFor'];
